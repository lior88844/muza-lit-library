...
async function fetchTracks() {
  try {
    ...
    console.warn('Failed to fetch tracks, using empty array:', (error as Error)?.message)
    return []
  }
}
...
async function fetchArtists() {
  try {
    ...
    console.warn('Failed to fetch artists, using empty array:', (error as Error)?.message)
    return []
  }
}
